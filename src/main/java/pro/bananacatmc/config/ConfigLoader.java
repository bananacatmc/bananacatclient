...
public class ConfigLoader {
    ...
    /**
     * Loads the config from .minecraft/BananaCat/config.json
     */

    public static void loadConfig() throws FileNotFoundException {
        FileReader reader = new FileReader(OSHelper.getBananaCatDirectory() + "config.json");

        Config config = new Gson().fromJson(reader, Config.class);

        for (int i = 0; i < config.getConfig().size(); i++) {
            Mod mod = BananaCat.INSTANCE.modManager.getMods().get(i);
            ...
            for (int j = 0; j < config.getConfig().get(i).getSettings().size(); j++) {
                ...
                Setting clientSetting = BananaCat.INSTANCE.settingManager.getSettingsByMod(mod).get(j);
            ...
            if (BananaCat.INSTANCE.hudEditor.getHudMod(mod.getName()) != null) {
                BananaCat.INSTANCE.hudEditor.getHudMod(mod.getName()).setX(config.getConfig().get(i).getPositions()[0]);
                BananaCat.INSTANCE.hudEditor.getHudMod(mod.getName()).setY(config.getConfig().get(i).getPositions()[1]);
                BananaCat.INSTANCE.hudEditor.getHudMod(mod.getName()).setSize(config.getConfig().get(i).getSize());
            }
        }

        for(int i = 0; i < config.getOptionsConfigList().size(); i++){
            Option configOption = config.getOptionsConfigList().get(i);
            Option clientOption = BananaCat.INSTANCE.optionManager.getOptions().get(i);
