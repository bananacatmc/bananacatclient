...
public abstract class RenderMixin {
    ...
    protected void renderLivingLabel(Entity entityIn, String str, double x, double y, double z, int maxDistance) {
        ...
        boolean nameTagToggled = BananaCat.INSTANCE.modManager.getMod("NameTag").isToggled();
        int color = BananaCat.INSTANCE.settingManager.getSettingByModAndName("NameTag", "Font Color").getColor().getRGB();
        float alpha = nameTagToggled ? BananaCat.INSTANCE.settingManager.getSettingByModAndName("NameTag", "Opacity").getCurrentNumber() / 255f : 0.25f;
        float scale = nameTagToggled ? BananaCat.INSTANCE.settingManager.getSettingByModAndName("NameTag", "Size").getCurrentNumber() : 1;
        float yPos = nameTagToggled ? BananaCat.INSTANCE.settingManager.getSettingByModAndName("NameTag", "Y Position").getCurrentNumber() - 2.5f : 0;
