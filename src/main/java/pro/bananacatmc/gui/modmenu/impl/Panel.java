...
package pro.bananacatmc.gui.modmenu.impl;
...
import pro.bananacatmc.gui.modmenu.impl.sidebar.options.type.*;
import pro.bananacatmc.gui.modmenu.impl.sidebar.TextBox;
import pro.bananacatmc.helpers.ResolutionHelper;
...
public class Panel {
    ...
    public void renderPanel(int mouseX, int mouseY) {
        ...
        Helper2D.drawPicture(x + 2, y - 1, 35, 35, color, "BananaCatlogo.png");
        BananaCat.INSTANCE.fontHelper.size40.drawString(BananaCat.modName, x + 37, y + 6, color);
        ...
        BananaCat.INSTANCE.messageHelper.renderMessage();

        if (selected == 0) {
            int offset = 0;
            for (Type type : Type.values()) {
                String text = type.name();
                int length = BananaCat.INSTANCE.fontHelper.size20.getStringWidth(text);
                ...
                BananaCat.INSTANCE.fontHelper.size20.drawString(text, x + offset + 26, y + h + 11, -1);
                offset += length + 30;
            }
        ...
        for (String button : sideButtons) {
            BananaCat.INSTANCE.fontHelper.size15.drawString(button, x - 30 - BananaCat.INSTANCE.fontHelper.size15.getStringWidth(button) / 2f, y + 30 + index * 40, color);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (mouseButton == 0) {
            ...
            if (MathHelper.withinBox(x + w - 25, y + 5, 20, 20, mouseX, mouseY)) {
                BananaCat.INSTANCE.mc.displayGuiScreen(BananaCat.INSTANCE.hudEditor);
            }
        }
