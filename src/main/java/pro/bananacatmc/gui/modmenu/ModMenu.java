...
package pro.bananacatmc.gui.modmenu;
...
import pro.bananacatmc.helpers.TimeHelper;
...
public class ModMenu extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        // Tăng 10% trong suốt
        Helper2D.drawRectangle(0, 0, width, height, 0x7F000000);
        ...
        BananaCat.INSTANCE.fontHelper.size40.drawString(TimeHelper.getFormattedTimeMinute(), width - 120, 10 - 50 + animateClock.getValueI(), color);
        BananaCat.INSTANCE.fontHelper.size20.drawString(TimeHelper.getFormattedDate(), width - 120, 30 - 50 + animateClock.getValueI(), color);

        // Vẽ nút snapping nếu cần
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (mouseButton == 0) {
            if (MathHelper.withinBox(panel.getX(), panel.getY(), panel.getW(), panel.getH(), mouseX, mouseY)) {
                panel.setDragging(true);
                panel.setOffsetX(mouseX - panel.getX());
                panel.setOffsetY(mouseY - panel.getY());
            }
            // Bỏ xử lý dark/light mode
        }
