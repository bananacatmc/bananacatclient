...
public class ReachdisplayHud extends HudMod {
    ...
    public void onAttack(AttackEntityEvent e) {
        if (BananaCat.INSTANCE.mc.objectMouseOver != null &&
                BananaCat.INSTANCE.mc.objectMouseOver.typeOfHit.equals(MovingObjectPosition.MovingObjectType.ENTITY)
        ) {
            Vec3 vec3 = BananaCat.INSTANCE.mc.getRenderViewEntity().getPositionEyes(1);
            range = BananaCat.INSTANCE.mc.objectMouseOver.hitVec.distanceTo(vec3);
        }
    }
        ...
        if (BananaCat.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                BananaCat.INSTANCE.fontHelper.size20.drawString(
                        ...
                        getX() + getW() / 2f - (BananaCat.INSTANCE.fontHelper.size20.getStringWidth(MathHelper.round(range, 2) + " Blocks")) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                BananaCat.INSTANCE.mc.fontRendererObj.drawString(
                        MathHelper.round(range, 2) + " Blocks",
                        getX() + getW() / 2 - (BananaCat.INSTANCE.mc.fontRendererObj.getStringWidth(MathHelper.round(range, 2) + " Blocks")) / 2,
                        getY() + 6,
                        getColor()
                );
            }
