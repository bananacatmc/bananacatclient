...
public class CpsHud extends HudMod {
        ...
        if (BananaCat.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                BananaCat.INSTANCE.fontHelper.size20.drawString(
                        ...
                        getX() + getW() / 2f - BananaCat.INSTANCE.fontHelper.size20.getStringWidth(isRightClick() ? "CPS: " + getLeftCPS() + " | " + getRightCPS() : "CPS: " + getLeftCPS()) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                BananaCat.INSTANCE.mc.fontRendererObj.drawString(
                        isRightClick() ? "CPS: " + getLeftCPS() + " | " + getRightCPS() : "CPS: " + getLeftCPS(),
                        getX() + getW() / 2 - BananaCat.INSTANCE.mc.fontRendererObj.getStringWidth(isRightClick() ? "CPS: " + getLeftCPS() + " | " + getRightCPS() : "CPS: " + getLeftCPS()) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private boolean isRightClick() {
        return BananaCat.INSTANCE.settingManager.getSettingByModAndName(getName(), "Right click").isCheckToggled();
    }

    private int getLeftCPS() {
        return BananaCat.INSTANCE.cpsHelper.getCPS(0);
    }

    private int getRightCPS() {
        return BananaCat.INSTANCE.cpsHelper.getCPS(1);
    }
}
