...
public class MouseKey {
    ...
    public void renderKey(int x, int y, int width, int height, boolean modern, int mouseButton, int color, int fontColor, boolean background, boolean cps) {
        ...
        if (modern) {
            ...
            BananaCat.INSTANCE.fontHelper.size20.drawString(
                    ...
                    x - BananaCat.INSTANCE.fontHelper.size20.getStringWidth(getCPS(mouseButton) != 0 && cps ? getCPS(mouseButton) + " CPS" : mouseButton == 0 ? "LMB" : "RMB") / 2f + width / 2f,
                    y + height / 2f - 4,
                    fontColor
            );
        }
    ...
    private int getCPS(int mouseButton) {
        return BananaCat.INSTANCE.cpsHelper.getCPS(mouseButton);
    }
}
