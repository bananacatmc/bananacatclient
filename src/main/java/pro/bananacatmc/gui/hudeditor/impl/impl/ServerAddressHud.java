        ...
        if (BananaCat.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                BananaCat.INSTANCE.fontHelper.size20.drawString(
                        BananaCat.INSTANCE.mc.isIntegratedServerRunning() ? "Singleplayer" : BananaCat.INSTANCE.mc.getCurrentServerData().serverIP,
                        getX() + getW() / 2f - (BananaCat.INSTANCE.fontHelper.size20.getStringWidth(BananaCat.INSTANCE.mc.isIntegratedServerRunning() ? "Singleplayer" : BananaCat.INSTANCE.mc.getCurrentServerData().serverIP)) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                BananaCat.INSTANCE.mc.fontRendererObj.drawString(
                        BananaCat.INSTANCE.mc.isIntegratedServerRunning() ? "Singleplayer" : BananaCat.INSTANCE.mc.getCurrentServerData().serverIP,
                        getX() + getW() / 2 - (BananaCat.INSTANCE.mc.fontRendererObj.getStringWidth(BananaCat.INSTANCE.mc.isIntegratedServerRunning() ? "Singleplayer" : BananaCat.INSTANCE.mc.getCurrentServerData().serverIP)) / 2,
                        getY() + 6,
                        getColor()
                );
            }
