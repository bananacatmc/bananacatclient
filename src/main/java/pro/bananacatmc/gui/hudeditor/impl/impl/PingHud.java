...
public class PingHud extends HudMod {
        ...
        if (BananaCat.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                BananaCat.INSTANCE.fontHelper.size20.drawString(
                        "Ping: " + getPing(),
                        getX() + getW() / 2f - BananaCat.INSTANCE.fontHelper.size20.getStringWidth("Ping: " + getPing()) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                BananaCat.INSTANCE.mc.fontRendererObj.drawString(
                        "Ping: " + getPing(),
                        getX() + getW() / 2 - BananaCat.INSTANCE.mc.fontRendererObj.getStringWidth("Ping: " + getPing()) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private long getPing() {
        return BananaCat.INSTANCE.mc.getCurrentServerData() == null ? 0 : BananaCat.INSTANCE.mc.getCurrentServerData().pingToServer;
    }
