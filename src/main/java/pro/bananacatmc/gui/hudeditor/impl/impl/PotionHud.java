...
public class PotionHud extends HudMod {
    ...
    public void onRender2D(RenderGameOverlayEvent.Pre.Text e) {
        GLHelper.startScale(getX(), getY(), getSize());
        Collection<PotionEffect> collection = BananaCat.INSTANCE.mc.thePlayer.getActivePotionEffects();
        if (BananaCat.INSTANCE.modManager.getMod(getName()).isToggled() && !(BananaCat.INSTANCE.mc.currentScreen instanceof HudEditor)) {
            if (isModern()) {
                ...
                int addY = 0;
                for (PotionEffect effect : BananaCat.INSTANCE.mc.thePlayer.getActivePotionEffects()) {
    ...
    private void drawPotion(Potion potion, PotionEffect effect, int y, boolean modern, boolean time) {
        if (modern) {
            BananaCat.INSTANCE.fontHelper.size20.drawString(I18n.format(potion.getName()), getX() + 30, time ? getY() + 5 + y : getY() + 10 + y, getColor());
            if (time) {
                BananaCat.INSTANCE.fontHelper.size20.drawString(Potion.getDurationString(effect), getX() + 30, getY() + 17 + y, getColor());
            }
        } else {
            BananaCat.INSTANCE.mc.fontRendererObj.drawString(I18n.format(potion.getName()), getX() + 30, time ? getY() + 5 + y : getY() + 10 + y, getColor());
            if (time) {
                BananaCat.INSTANCE.mc.fontRendererObj.drawString(Potion.getDurationString(effect), getX() + 30, getY() + 17 + y, getColor());
            }
        }
        BananaCat.INSTANCE.mc.getTextureManager().bindTexture(inventoryBackground);
    ...
    private boolean isTime() {
        return BananaCat.INSTANCE.settingManager.getSettingByModAndName(getName(), "Show Time").isCheckToggled();
    }
}
