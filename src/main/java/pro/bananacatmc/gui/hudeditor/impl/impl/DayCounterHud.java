...
public class DayCounterHud extends HudMod {
        ...
        if (BananaCat.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                BananaCat.INSTANCE.fontHelper.size20.drawString(
                        "Day: " + getDay(),
                        getX() + getW() / 2f - BananaCat.INSTANCE.fontHelper.size20.getStringWidth("Day: " + getDay()) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                BananaCat.INSTANCE.mc.fontRendererObj.drawString(
                        "Day: " + getDay(),
                        getX() + getW() / 2 - BananaCat.INSTANCE.mc.fontRendererObj.getStringWidth("Day: " + getDay()) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private long getDay() {
        return BananaCat.INSTANCE.mc.theWorld.getWorldTime() / 24000L;
    }
}
